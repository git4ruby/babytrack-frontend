...
const canSubmit = computed(() => {
  if (feedType.value === 'bottle' || feedType.value === 'pump') {
    return volumeMl.value > 0
  }
  if (feedType.value === 'breastfeed') {
    return !!breastSide.value
  }
  return false
})
...
async function submit() {
  ...
  try {
    ...
    ui.showToast('Feed logged successfully')
    close()
    ...
    ui.showToast(e.response?.data?.errors?.[0] || 'Failed to log feed', 'error')
  } finally {
    loading.value = false
  }
}
...
<template>
  <BaseModal :open="ui.feedModalOpen" title="Log Feed" @close="close">
    <!-- Step 1: Select Type -->
    <div v-if="step === 1" class="space-y-3 mt-2">
      <button
    ...
    <div v-if="step === 2" class="space-y-4 mt-2">
      ...
      <div class="flex items-center gap-2">
        <button @click="goBack" class="text-sm text-blue-600 hover:text-blue-700">← Back</button>
      ...
      <template v-if="feedType === 'bottle' || feedType === 'pump'">
        ...
        <div v-if="feedType === 'bottle'">
          ...
          <div class="flex gap-2">
            <button
              v-for="mt in [{ v: 'breast_milk', l: 'Breast Milk' }, { v: 'formula', l: 'Formula' }, { v: 'mixed', l: 'Mixed' }]"
              :key="mt.v"
              @click="milkType = mt.v"
              :class="[
                'flex-1 py-2 text-sm font-medium rounded-lg border-2 transition',
                milkType === mt.v ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'
              ]"
            >
              {{ mt.l }}
            </button>
        ...
        <div v-if="milkType === 'formula'">
          ...
          <input
            v-model="formulaBrand"
            type="text"
            placeholder="e.g. Similac 360 Total Care"
            class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm"
          />
        </div>
      </template>
        ...
        <div>
          ...
          <div class="flex gap-2">
            <button
              ...
              :key="side"
              @click="breastSide = side"
              :class="[
                'flex-1 py-2.5 text-sm font-medium rounded-lg border-2 transition capitalize',
                breastSide === side ? 'border-pink-500 bg-pink-50 text-pink-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'
              ]"
            >
              {{ side }}
            </button>
      ...
      <div>
        ...
        <input
          v-model="notes"
          type="text"
          placeholder="e.g. fussy after feed"
          class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm"
        />
      </div>

      <!-- Combo feed toggle (bottle only) -->
      <div v-if="feedType === 'bottle'" class="border-t border-gray-100 pt-4">
        ...
        <div v-if="addCombo" class="mt-3 pl-6 space-y-3">
          ...
          <div class="flex gap-2">
            <button
              v-for="mt in [{ v: 'breast_milk', l: 'Breast Milk' }, { v: 'formula', l: 'Formula' }]"
              :key="mt.v"
              @click="comboMilkType = mt.v"
              :class="[
                'flex-1 py-2 text-sm font-medium rounded-lg border-2 transition',
                comboMilkType === mt.v ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'
              ]"
            >
              {{ mt.l }}
            </button>
