  ...
  <BaseModal :open="ui.feedModalOpen" :title="modalTitle" @close="close">
    ...
    <div v-if="step === 1 && !isEditing" class="space-y-3 mt-2">
      <button
        ...
        class="w-full flex items-center gap-4 p-4 rounded-xl border-2 border-gray-100 hover:border-blue-200 hover:bg-blue-50 transition text-left"
      >
        <span class="text-3xl">{{ ft.icon }}</span>
        <div>
          <p class="font-medium text-gray-900">{{ ft.label }}</p>
          <p class="text-sm text-gray-500">{{ ft.desc }}</p>
        </div>
      </button>
    </div>
    ...
    <div v-if="step === 2" class="space-y-4 mt-2">
      <div class="flex items-center gap-2">
        ...
        <span class="text-sm font-medium text-gray-700 capitalize">{{ feedType }}</span>
      </div>

      <!-- Time -->
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Start Time</label>
        <input v-model="startedAt" type="datetime-local" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm" />
      </div>
      ...
      <template v-if="feedType === 'bottle' || feedType === 'pump'">
        <div>
          ...
          <input v-model="volumeMl" type="number" min="1" max="500" placeholder="e.g. 60" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm" />
        </div>
        <div v-if="feedType === 'bottle'">
          <label class="block text-sm font-medium text-gray-700 mb-2">Milk Type</label>
          <div class="flex gap-2">
            <button v-for="mt in [{v:'breast_milk',l:'Breast Milk'},{v:'formula',l:'Formula'},{v:'mixed',l:'Mixed'}]" :key="mt.v" @click="milkType = mt.v" :class="['flex-1 py-2 text-sm font-medium rounded-lg border-2 transition', milkType === mt.v ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-600']">{{ mt.l }}</button>
          </div>
        </div>
        <div v-if="milkType === 'formula'">
          <label class="block text-sm font-medium text-gray-700 mb-1">Formula Brand</label>
          <input v-model="formulaBrand" type="text" placeholder="e.g. Similac 360 Total Care" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm" />
        </div>
      </template>
      ...
      <template v-if="feedType === 'breastfeed'">
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-2">Breast Side</label>
          <div class="flex gap-2">
            <button v-for="side in ['left','right','both']" :key="side" @click="breastSide = side" :class="['flex-1 py-2.5 text-sm font-medium rounded-lg border-2 transition capitalize', breastSide === side ? 'border-pink-500 bg-pink-50 text-pink-700' : 'border-gray-200 text-gray-600']">{{ side }}</button>
          </div>
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">End Time (optional)</label>
          <input v-model="endedAt" type="datetime-local" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm" />
        </div>
      </template>
      ...
      <div>
        ...
        <input v-model="notes" type="text" placeholder="e.g. fussy after feed" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm" />
      </div>

      <!-- Combo (new only) -->
      <div v-if="feedType === 'bottle' && !isEditing" class="border-t border-gray-100 pt-4">
        <label class="flex items-center gap-2 cursor-pointer">
          <input v-model="addCombo" type="checkbox" class="w-4 h-4 text-blue-600 rounded border-gray-300" />
          <span class="text-sm text-gray-700">Add a second bottle (combo feed)</span>
        </label>
        <div v-if="addCombo" class="mt-3 pl-6 space-y-3">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Second Volume (ml)</label>
            <input v-model="comboVolume" type="number" min="1" max="500" placeholder="e.g. 30" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm" />
          </div>
          <div class="flex gap-2">
            <button v-for="mt in [{v:'breast_milk',l:'Breast Milk'},{v:'formula',l:'Formula'}]" :key="mt.v" @click="comboMilkType = mt.v" :class="['flex-1 py-2 text-sm font-medium rounded-lg border-2 transition', comboMilkType === mt.v ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 text-gray-600']">{{ mt.l }}</button>
          </div>
        </div>
      </div>
