...
<template>
  <div class="space-y-5">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
      ...
      <p class="text-xs text-gray-400 mt-2">{{ progress }}% complete</p>
    </div>
    ...
    <div class="flex gap-1 bg-gray-100 p-1 rounded-lg">
      <button
        ...
        :class="['flex-1 py-2 text-sm font-medium rounded-md transition', activeTab === 'upcoming' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500']"
      >
        Upcoming ({{ upcoming.length }})
      </button>
      <button
        @click="activeTab = 'completed'"
        :class="['flex-1 py-2 text-sm font-medium rounded-md transition', activeTab === 'completed' ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500']"
      >
    ...
    <template v-if="activeTab === 'upcoming'">
      <div v-for="group in groupedUpcoming" :key="group.label" class="space-y-2">
        <h3 class="text-sm font-semibold text-gray-700 px-1">{{ group.label }}</h3>
        <div class="space-y-2">
          <div
            ...
            :class="[
              ...
              vax.overdue ? 'border-red-200 bg-red-50' : 'border-gray-100'
            ]"
          >
            <div class="flex items-start gap-3">
              ...
              <div>
                <p class="font-medium text-gray-900 text-sm">{{ vax.vaccine_name }}</p>
                <p class="text-xs text-gray-500 mt-0.5">{{ vax.description }}</p>
                <p v-if="vax.recommended_date" class="text-xs mt-1" :class="vax.overdue ? 'text-red-600 font-medium' : 'text-gray-400'">
    ...
    <template v-if="activeTab === 'completed'">
      <div class="space-y-2">
        <div v-for="vax in completed" :key="vax.id" class="bg-white rounded-xl shadow-sm border border-gray-100 p-4 flex items-center gap-3">
          ...
          <div class="flex-1">
            ...
            <p v-if="vax.lot_number" class="text-xs text-gray-400">Lot: {{ vax.lot_number }}</p>
          </div>
        </div>
      </div>
      <p v-if="!completed.length" class="text-center text-sm text-gray-400 py-8">No completed vaccinations yet</p>
    </template>
    ...
    <BaseModal :open="adminModal.open" :title="'Mark ' + (adminModal.vax?.vaccine_name || '') + ' as Administered'" @close="adminModal = { open: false, vax: null }">
      <div class="space-y-4 mt-2">
        <div>
          ...
          <input v-model="adminForm.administered_at" type="date" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Administered By</label>
          <input v-model="adminForm.administered_by" type="text" placeholder="e.g. Dr. Smith" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm" />
        </div>
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Lot Number</label>
            <input v-model="adminForm.lot_number" type="text" placeholder="Optional" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm" />
          </div>
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-1">Site</label>
            <input v-model="adminForm.site" type="text" placeholder="e.g. Left thigh" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm" />
          </div>
        </div>
