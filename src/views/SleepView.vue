...
<template>
  <div class="space-y-5">
    <div class="flex items-center justify-between">
      <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Sleep</h1>
    ...
    <div class="flex flex-wrap gap-2">
      <button v-for="preset in rangePresets" :key="preset.value" @click="rangePreset = preset.value"
        :class="['px-3 py-1.5 text-sm font-medium rounded-lg transition-all', rangePreset === preset.value ? 'bg-slate-900 dark:bg-slate-600 text-white shadow-md' : 'bg-white dark:bg-slate-800 text-gray-600 dark:text-slate-300 border border-gray-200 dark:border-slate-700 hover:bg-gray-100 dark:hover:bg-slate-700']">
        {{ preset.label }}
      </button>
    </div>
    ...
    <template v-if="groupedLogs.length">
      <div v-for="[date, sleeps] in groupedLogs" :key="date" class="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
        <div class="flex items-center justify-between px-5 py-3 bg-gradient-to-r from-slate-50 to-gray-50 dark:from-slate-800 dark:to-slate-800 border-b border-gray-100 dark:border-slate-700">
          ...
          <div class="flex items-center gap-2 text-xs">
            <span class="font-bold text-indigo-600 dark:text-indigo-400">{{ Math.round(dayTotalMin(sleeps) / 60 * 10) / 10 }}h sleep</span>
            <span class="text-gray-400 dark:text-slate-500">{{ sleeps.length }} sessions</span>
          </div>
        </div>
          ...
          <div v-for="log in sleeps" :key="log.id" @click="openEdit(log)" class="flex items-center gap-3 py-3 group cursor-pointer hover:bg-gray-50 dark:hover:bg-slate-700 -mx-4 px-4 rounded-lg transition">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2">
                ...
                <span v-if="log.location" class="text-[10px] px-1.5 py-0.5 bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-slate-300 rounded-md font-bold uppercase">{{ log.location }}</span>
              </div>
              ...
              <p v-if="log.notes" class="text-xs text-gray-400 dark:text-slate-500 italic mt-0.5">{{ log.notes }}</p>
            </div>
    ...
    <BaseModal :open="showForm" :title="editingId ? 'Edit Sleep' : 'Log Sleep'" @close="showForm = false">
      <div class="space-y-4 mt-2">
          ...
          <div class="flex gap-2">
            <button v-for="(cfg, type) in typeConfig" :key="type" @click="form.sleep_type = type"
              :class="['flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border-2 transition',
                form.sleep_type === type ? `border-${cfg.color}-500 bg-${cfg.color}-50 dark:bg-${cfg.color}-900/30` : 'border-gray-200 dark:border-slate-600']">
        ...
        <div>
          ...
          <input v-model="form.started_at" type="datetime-local" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-slate-600 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm bg-white dark:bg-slate-700 dark:text-white" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-slate-200 mb-1">End Time (optional — fill when baby wakes)</label>
          <input v-model="form.ended_at" type="datetime-local" class="w-full px-3 py-2.5 rounded-lg border border-gray-300 dark:border-slate-600 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none text-sm bg-white dark:bg-slate-700 dark:text-white" />
        </div>
        <div>
          <label class="block text-sm font-medium text-gray-700 dark:text-slate-200 mb-2">Location (optional)</label>
          <div class="flex flex-wrap gap-2">
            <button v-for="loc in locations" :key="loc" @click="form.location = form.location === loc ? '' : loc"
              :class="['px-3 py-1.5 text-xs font-medium rounded-full border-2 capitalize transition',
                form.location === loc ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-400' : 'border-gray-200 dark:border-slate-600 text-gray-600 dark:text-slate-300']">
              {{ loc }}
            </button>
          </div>
        </div>
