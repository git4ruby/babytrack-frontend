...
<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      ...
      <!-- Quick Actions -->
      <div class="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-slate-700 flex flex-col">
        <p class="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-3">Quick Actions</p>
        <div class="grid grid-cols-2 gap-2 flex-1">
          <button @click="ui.feedModalOpen = true" class="flex flex-col items-center justify-center gap-1.5 rounded-xl bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition group">
            <span class="text-2xl group-hover:scale-110 transition-transform">🍼</span>
            <span class="text-[11px] font-bold text-blue-700 dark:text-blue-400">Log Feed</span>
          </button>
          <button @click="ui.milkModalOpen = true" class="flex flex-col items-center justify-center gap-1.5 rounded-xl bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 transition group">
            <span class="text-2xl group-hover:scale-110 transition-transform">🧊</span>
            <span class="text-[11px] font-bold text-indigo-700 dark:text-indigo-400">Store Milk</span>
          </button>
          <button @click="ui.diaperModalOpen = true" class="flex flex-col items-center justify-center gap-1.5 rounded-xl bg-cyan-50 dark:bg-cyan-900/30 hover:bg-cyan-100 dark:hover:bg-cyan-900/50 transition group">
            <span class="text-2xl group-hover:scale-110 transition-transform">🧷</span>
            <span class="text-[11px] font-bold text-cyan-700 dark:text-cyan-400">Diaper</span>
          </button>
          <router-link to="/weight" class="flex flex-col items-center justify-center gap-1.5 rounded-xl bg-emerald-50 dark:bg-emerald-900/30 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 transition group">
            <span class="text-2xl group-hover:scale-110 transition-transform">⚖️</span>
            <span class="text-[11px] font-bold text-emerald-700 dark:text-emerald-400">Weight</span>
          </router-link>
        </div>
      </div>
    </div>
    ...
    <div v-if="summary" class="grid grid-cols-2 lg:grid-cols-5 gap-3">
      ...
      <router-link to="/diapers" class="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-slate-700 text-center hover:border-cyan-200 dark:hover:border-cyan-700 transition">
        ...
        <p v-if="diaperSummary && diaperSummary.total > 0" class="text-[10px] text-gray-400 dark:text-slate-500 mt-0.5">💧{{ diaperSummary.wet }} 💩{{ diaperSummary.soiled }}</p>
      </router-link>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <!-- Weekly mini chart -->
      <div v-if="Object.keys(weeklyTotals).length" class="lg:col-span-2 bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-slate-700">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider">Last 7 Days — Daily Volume</h3>
        ...
        <div class="flex items-end gap-3 h-28">
          <div v-for="(d, i) in sparkData" :key="d.date" class="flex-1 flex flex-col items-center gap-1">
            <span class="text-[10px] font-bold text-gray-500 dark:text-slate-400">{{ d.ml || '' }}</span>
            <div
              :class="[
      ...
      <!-- Feed type + balance -->
      <div v-if="summary" class="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-slate-700">
        <h3 class="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-4">Today's Mix</h3>
        <div class="space-y-3">
          <div class="flex items-center justify-between">
            <span class="text-sm text-gray-600 dark:text-slate-300">🍼 Bottles</span>
            <span class="text-sm font-black text-blue-600">{{ summary.feeds_by_type?.bottle || 0 }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-sm text-gray-600 dark:text-slate-300">🤱 Breastfeed</span>
            <span class="text-sm font-black text-pink-600">{{ summary.feeds_by_type?.breastfeed || 0 }}</span>
          </div>
          <div class="flex items-center justify-between">
            <span class="text-sm text-gray-600 dark:text-slate-300">⚙️ Pump</span>
            <span class="text-sm font-black text-purple-600">{{ summary.feeds_by_type?.pump || 0 }}</span>
          </div>
        </div>
        <!-- Breast Balance -->
        <div v-if="summary.breast_balance && (summary.breast_balance.left_percent > 0 || summary.breast_balance.right_percent > 0)" class="mt-4 pt-4 border-t border-gray-100 dark:border-slate-700">
          <p class="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider mb-2">Breast Balance</p>
          <div class="flex h-5 rounded-full overflow-hidden bg-gray-100 dark:bg-slate-700">
          ...
          <div class="flex justify-between text-[10px] text-gray-400 dark:text-slate-500 mt-1 font-medium">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
      <!-- Milk Inventory -->
      <div v-if="inventory" class="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-slate-700">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-wider">Milk Inventory</h3>
          <router-link to="/milk-storage" class="text-xs text-blue-600 hover:text-blue-700 font-semibold">Manage →</router-link>
        </div>
        <div class="space-y-2.5">
          <div class="flex items-center justify-between p-2.5 rounded-xl bg-orange-50 dark:bg-orange-900/30">
            <span class="text-sm dark:text-slate-200">🌡️ Room Temp</span>
            <span class="text-sm font-black text-orange-600">{{ inventory.by_storage_type?.room_temp?.total_ml || 0 }}ml</span>
          </div>
          <div class="flex items-center justify-between p-2.5 rounded-xl bg-sky-50 dark:bg-sky-900/30">
            <span class="text-sm dark:text-slate-200">❄️ Fridge</span>
            <span class="text-sm font-black text-sky-600">{{ inventory.by_storage_type?.fridge?.total_ml || 0 }}ml</span>
          </div>
          <div class="flex items-center justify-between p-2.5 rounded-xl bg-indigo-50 dark:bg-indigo-900/30">
            <span class="text-sm dark:text-slate-200">🧊 Freezer</span>
            <span class="text-sm font-black text-indigo-600">{{ inventory.by_storage_type?.freezer?.total_ml || 0 }}ml</span>
          </div>
        </div>
        <div class="mt-3 text-center">
          <span class="text-lg font-black text-gray-800 dark:text-white">{{ inventory.summary?.total_ml || 0 }}ml</span>
          <span class="text-xs text-gray-400 dark:text-slate-500"> total</span>
        </div>
      ...
      <!-- Next Appointment -->
      <div v-if="nextAppointment" class="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/20 rounded-2xl p-5 border border-amber-200/50 dark:border-amber-700/50">
        <h3 class="text-xs font-bold text-amber-500 uppercase tracking-wider mb-3">Next Appointment</h3>
        <p class="font-bold text-gray-900 dark:text-white text-lg">{{ nextAppointment.title }}</p>
        <p class="text-sm text-gray-600 dark:text-slate-300 mt-2">📅 {{ dayjs(nextAppointment.scheduled_at).format('ddd, MMM D · h:mm A') }}</p>
        <p v-if="nextAppointment.provider_name" class="text-xs text-gray-500 dark:text-slate-400 mt-1">👨‍⚕️ {{ nextAppointment.provider_name }}</p>
        <p v-if="nextAppointment.location" class="text-xs text-gray-500 dark:text-slate-400">📍 {{ nextAppointment.location }}</p>
      ...
      <div v-else class="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-slate-700 flex flex-col items-center justify-center text-center">
        <span class="text-3xl mb-2">📅</span>
        <p class="text-sm text-gray-400 dark:text-slate-500">No upcoming appointments</p>
        <router-link to="/appointments" class="text-xs text-blue-600 font-semibold mt-2">Schedule one →</router-link>
      </div>

      <!-- Upcoming Vaccines -->
      <div v-if="upcomingVaccines.length" class="bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 rounded-2xl p-5 border border-emerald-200/50 dark:border-emerald-700/50">
        ...
        <div class="space-y-2">
          <div v-for="vax in upcomingVaccines" :key="vax.id" class="flex items-center justify-between bg-white/60 dark:bg-white/10 rounded-lg p-2">
            <span class="text-xs text-gray-700 dark:text-slate-200 font-medium">{{ vax.vaccine_name }}</span>
      ...
      <div v-else class="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-100 dark:border-slate-700 flex flex-col items-center justify-center text-center">
        <span class="text-3xl mb-2">💉</span>
        <p class="text-sm text-gray-400 dark:text-slate-500">Vaccines up to date</p>
